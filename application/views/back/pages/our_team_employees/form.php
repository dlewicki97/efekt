    ...
    <div class="br-pagebody mg-t-0 pd-x-30">
        ...
        <form class="form-layout form-layout-6" action="<?php echo base_url() . 'panel/' . $this->uri->segment(2) . '/action/' . $this->uri->segment(4) . '/' . $this->uri->segment(2); ?>/<?php echo @$value->id; ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="row no-gutters">
                ...
                <div class="col-12 col-sm-8 col-lg-9">
                    <input class="form-control" type="text" name="work_place" value="<?= @$value->work_place; ?>">
                </div>
            </div>
            <div class="row no-gutters">
                <div class="col-12 col-sm-4 col-lg-2">
                    Wybierz grupę pracowniczą:
                </div>
                <div class="col-12 col-sm-8 col-lg-9">
                    <select class="form-control select2-show-search" name="our_team_employee_group_id">
                        <?php foreach ($this->back_m->get_all('our_team_employee_groups') as $group) : ?>
                            <option <?php if ($group->id == @$value->our_team_employee_group_id) echo 'selected' ?> value="<?= $group->id ?>"><?= $group->title ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
