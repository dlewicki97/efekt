                ...
                <table id="datatable1" class="table display responsive nowrap">
                    <thead>
                        <tr>
                            ...
                            <th class="wd-5p align-top">Numer</th>
                            <th class="wd-10p align-top no-sort">Aktywny</th>
                            <th class="wd-10p align-top no-sort">Strona główna</th>
                            <th class="wd-35p align-top">Miasto</th>
                        ...
                        foreach ($rows as $value) : $i++; ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <label class="ckbox">
                                        <input type="checkbox" id="active<?= $value->id ?>" onchange="setCheckbox(<?= $value->id ?>, 'active', '<?= $this->uri->segment(2); ?>');" <?php if (@$value->active) echo "checked"; ?>>
                                        <span></span>
                                    </label>
                                </td>
                                <td class="align-middle">
                                    <label class="ckbox">
                                        <input type="checkbox" id="home<?= $value->id ?>" onchange="setCheckbox(<?= $value->id ?>, 'home', '<?= $this->uri->segment(2); ?>');" <?php if (@$value->home) echo "checked"; ?>>
                                        <span></span>
                                    </label>
                                </td>
                                <td class="align-middle"><?= substr($value->city, 0, 70);
                                                            if (strlen($value->city) > 70) echo '...' ?></td>
                                <td class="text-right">
                                    ...
                                    <a href="<?= base_url(); ?>panel/settings/delete/<?= $this->uri->segment(2); ?>/<?= $value->id; ?>" class="btn btn-sm btn-secondary" onclick="return confirm('Czy na pewno chcesz usunąć <?= $value->city; ?>? #<?= $value->id; ?>')">
