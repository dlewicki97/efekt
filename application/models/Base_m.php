...
class Base_m extends CI_Model
{
    ...
    {
        return $this->db->query('CREATE TABLE logs (
	    	id int NOT NULL AUTO_INCREMENT,
	    	created DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, 
	    	ip TEXT,
	    	browser TEXT,
    ...
    public function create_base()
    {
        ...
        $query = $this->db->query('CREATE TABLE `settings` (
                                    ...
                                    `modal_photos` int(11) NULL
                                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;');

        $query = $this->db->query('INSERT INTO `settings` (`id`, `meta_title`, `description`, `keywords`, `privace`, `logo`,                                `fb_link`, `inst_link`, `yt_link`, `tw_link`, `modal_photos`) VALUES
                                    (1, "", "", "", "", "", "", "", "", "", 2);');
