...
class Settings extends CI_Controller
{
	...
	public function add_photo()
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			addMedia($data);
			$insert[$field] = $now . '/' . $data['file_name'];
			$this->back_m->update($table, $insert, 1);
		} else {
			redirect('panel');
		}
	}
	...
	public function footer_photo()
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$this->load->library('upload', $config);
			$this->upload->initialize($config);
			$this->upload->do_upload('photo_footer_logo');
			$data = $this->upload->data();
			addMedia($data);
			$insert['footer_logo'] = $now . '/' . $data['file_name'];
			if ($data['image_width'] > 1440) {
				resizeImg($data['file_name'], $now, '1440');
			}
			$this->back_m->update($_POST['table'], $insert, 1);
	...
	public function delete($table, $id)
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$irremovable = $this->config->item('irremovableTables');
			if (!in_array($table, $irremovable)) {
	...
	public function setCheckbox()
	{
		if (checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			if (!$this->db->field_exists($_POST['field'], $_POST['table'])) {
				$this->base_m->create_boolean_column($_POST['table'], $_POST['field']);
			}
			$insert[$_POST['field']] = $_POST['value'];
			$this->back_m->update($_POST['table'], $insert, $_POST['id']);
		}
